...
const trendingCardVariants = cva(
  "shrink-0 w-70 transition-all duration-250 ease-out",
  {
    variants: {
      state: {
        active: "scale-105 opacity-100 z-10",
        inactive: "scale-95 opacity-75",
      },
      motion: {
        enabled: "",
        disabled: "!transition-none !scale-100 !opacity-100",
      },
    },
    defaultVariants: {
      state: "inactive",
  ...
  "h-2 rounded-full transition-all duration-300",
  {
    variants: {
      state: {
        ...
        inactive: "w-2 bg-gray-300 hover:bg-gray-400",
      },
    },
    defaultVariants: {
      state: "inactive",
...
function TrendingProductCard({
  ...
  return (
    <div
      className={cn(
        trendingCardVariants({
          state: isActive ? "active" : "inactive",
          motion: prefersReducedMotion ? "disabled" : "enabled",
        })
      )}
      style={{
      ...
      <div className="group relative flex flex-col bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-lg">
        ...
        <div className="flex flex-1 flex-col p-4 text-center">
          ...
          {product.category && (
            <Text
              variant="caption"
              className="font-medium text-gray-400 uppercase tracking-wide mb-1"
            >
              {product.category}
            </Text>
          )}
          ...
          <div className="flex items-baseline justify-center gap-2 flex-wrap">
            ...
            {hasDiscount && originalPrice && (
              <span className="relative text-sm text-gray-400 font-medium px-1.5 py-0.5 rounded-sm bg-linear-to-r from-gray-100/50 to-transparent">
                {formatPrice(originalPrice)}
                {/* Modern diagonal strike-through */}
                <span className="absolute inset-0 flex items-center justify-center">
                  <span className="w-full h-px bg-linear-to-r from-transparent via-gray-400 to-transparent transform rotate-12 origin-center opacity-80"></span>
                </span>
              </span>
            )}
          </div>
...
export function TrendingNow({
  ...
  useEffect(() => {
    ...
    // Initial active index calculation - use the logic directly instead of the callback
  ...
  }, [handleScroll, handleWheel, products.length]);
    ...
    <Section spacing="lg">
      <div className="space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          <Heading level="h2" className="text-neutral-900">
            {title}
          </Heading>
          <Text className="text-lg text-neutral-600 max-w-2xl mx-auto">
            {subtitle}
          </Text>
        </div>
