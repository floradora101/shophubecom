...
import { normalizeAdminProductsFilters } from "../utils/normalize-filters";
...
export function useAdminProductsQuery(params?: ProductsListParams) {
  ...
  const normalizedParams = normalizeAdminProductsFilters(params);

  return useQuery<ProductsListResult>({
    queryKey: adminQueryKeys.products.list(normalizedParams),
    queryFn: () => adminProductsApi.getProducts(normalizedParams),
