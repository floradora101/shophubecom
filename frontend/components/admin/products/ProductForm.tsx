...
import type { Product, VariantOptions } from "@/lib/types/product.types";
import {
  productSchema,
...
export function ProductForm({ product, onSave }: ProductFormProps) {
  ...
  const onError = (validationErrors: Record<string, { message?: string }>) => {
    // Log all errors for debugging (expand the object to see nested errors)
    console.error(
    ...
    // Check for top-level field errors first
    ...
    // Check for variant errors
    if (validationErrors.variants) {
      const variantErrors = validationErrors.variants as unknown;
      if (Array.isArray(variantErrors)) {
        variantErrors.forEach((variantError, index) => {
          if (variantError && typeof variantError === "object") {
            // Check for variantHasImage test error (yup test creates { message, type } structure)
            if (
              "type" in variantError &&
              (variantError as { type?: string }).type === "variantHasImage"
            ) {
              const variantData = watch(`variants.${index}`);
              console.error(
                `[Validation] Variant ${index + 1} image validation failed:`,
                {
                  image: variantData?.image,
                  images: variantData?.images,
                  fullVariantData: variantData,
                }
              );
              errorMessages.push(
                `Variant ${index + 1}: ${
                  (variantError as { message?: string }).message ||
                  "Add at least one image (main image or gallery)"
                }`
              );
            }
            // Also check for nested variantHasImage error (fallback)
            if ("variantHasImage" in variantError) {
              const imgError = (
                variantError as { variantHasImage?: { message?: string } }
              ).variantHasImage;
              errorMessages.push(
                `Variant ${index + 1}: ${
                  imgError?.message ||
            ...
            if (
              "image" in variantError &&
              (variantError as { image?: { message?: string } }).image?.message
            ) {
              errorMessages.push(
                `Variant ${index + 1} - Main Image: ${
                  (variantError as { image?: { message?: string } }).image
                    ?.message
                }`
              );
            }
            if (
              "sku" in variantError &&
              (variantError as { sku?: { message?: string } }).sku?.message
            ) {
              errorMessages.push(
                `Variant ${index + 1} - SKU: ${
                  (variantError as { sku?: { message?: string } }).sku?.message
            ...
            if (
              "price" in variantError &&
              (variantError as { price?: { message?: string } }).price?.message
            ) {
              errorMessages.push(
                `Variant ${index + 1} - Price: ${
                  (variantError as { price?: { message?: string } }).price
                    ?.message
                }`
              );
            }
            if (
              "stock" in variantError &&
              (variantError as { stock?: { message?: string } }).stock?.message
            ) {
              errorMessages.push(
                `Variant ${index + 1} - Stock: ${
                  (variantError as { stock?: { message?: string } }).stock
                    ?.message
                }`
              );
            }
    ...
    if (errorMessages.length > 0) {
      ...
      setTimeout(() => {
        const firstErrorField = document.querySelector(
          '[class*="border-red-500"], .ut-button, input[aria-invalid="true"]'
        );
        if (firstErrorField) {
      ...
      const errorText = `Validation failed. Check console for details. Errors: ${Object.keys(
        validationErrors
      ).join(", ")}`;
      setFormError(errorText);
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    {errors.variants?.[index]?.stock && (
                      <p className="mt-1 text-xs text-red-600">
                        {errors.variants[index]?.stock?.message as string}
                      </p>
                    )}
                  </div>
                </div>
