...
export const adminApi = {
  ...
  deleteCarouselSlide: async (
    ...
    const index = mockCarouselSlides.findIndex((s) => s.id === slideId);
    if (index !== -1) {
      mockCarouselSlides.splice(index, 1);
    }
    return { success: true };
  },
  ...
  reorderCarouselSlides: async (
    ...
    const slides = mockCarouselSlides.filter(
      (s) => s.carouselTypeId === carouselTypeId
    );
  ...
  deleteCustomBanner: async (
    ...
    await delay(300);
    const index = mockCustomBanners.findIndex((b) => b.id === bannerId);
    if (index !== -1) {
      mockCustomBanners.splice(index, 1);
    }
    return { success: true };
  },
    ...
    let products: AdminProduct[] = getProducts().map((p) => {
      const convertedVariants: ProductVariant[] = (p.variants || []).map(
        (v, idx) => {
          ...
          const options =
            (legacy.options as Record<string, string> | undefined) ||
            (() => {
              const derived: Record<string, string> = {};
              if (legacy.color) derived.color = String(legacy.color);
              if (legacy.storage) derived.storage = String(legacy.storage);
              return Object.keys(derived).length ? derived : undefined;
            })();
          return {
            id: String(legacy.id || ""),
            sku: String(legacy.sku || legacy.id || `${p.slug}-var-${idx + 1}`),
            price:
              typeof legacy.price === "number" && legacy.price > 0
                ? legacy.price
                : p.price,
            stock: (legacy.stock as number) ?? 0,
            image: legacy.image as string | undefined,
      ...
      // Handle images - check if product has images property (may be in extended type)
